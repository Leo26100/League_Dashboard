...
```{r Unique Picks Per Team echo=FALSE}
Picks_df %>%
  group_by(Role, Team)%>%
  count(Champions) %>%
  ggplot()+
...
```
```{r Unique Bot Champions per Team echo = FALSE}
Picks_df %>%
  group_by(Role, Team)%>%
  filter(Role == 'Bot Laner')
  count(Champions) %>%
...
```{r Bot Lane Champions Count echo=FALSE}
...
```
So Kai'sa and Xayah always? Yup, I really do think that in terms of champion diversity riot failed when it came to Bot lane.
